...
def get_attn_params(attn_size, initializer=tf.truncated_normal_initializer):
    ...
    with tf.variable_scope("attention_weights"):
        params = {
            ...
            # "W_c1": tf.get_variable("W_c1", dtype=tf.float32, shape=(2 * attn_size, 2 * attn_size),
            #                         initializer=eye_constant_initializer),
            # "W_c2": tf.get_variable("W_c2", dtype=tf.float32, shape=(2 * attn_size, 2 * attn_size),
            #                         initializer=eye_constant_initializer),
            # "W_c3": tf.get_variable("W_c3", dtype=tf.float32, shape=(2 * attn_size, 2 * attn_size),
            #                         initializer=eye_constant_initializer),
            # "W_c4": tf.get_variable("W_c4", dtype=tf.float32, shape=(2 * attn_size, 2 * attn_size),
            #                         initializer=eye_constant_initializer),

            # Store layers weight & biases
            'h1': tf.get_variable("h1", dtype=tf.float32, shape=(2*attn_size, attn_size),
                                  initializer=initializer()),
            ...
            "W_h_P": tf.get_variable("W_h_P", dtype=tf.float32, shape=(2 * attn_size, 2*attn_size),
                                     initializer=initializer()),
            "W_h_a": tf.get_variable("W_h_a", dtype=tf.float32, shape=(2 * attn_size, 2*attn_size),
                                     initializer=initializer()),
...
#             with tf.variable_scope(scope):
#                 if i > 0: tf.get_variable_scope().reuse_variables()
#                 video_slice = tf.reshape(tf.slice(video_fts, begin=[0, i, 0], size=[-1, 1, -1]),
#                                          [Params.batch_size, -1])
#                 attention_inputs = [sentence_fts, video_slice]
#                 attention_weights = attention(attention_inputs, Params.attn_size, Weights, scope="attention",
#                                               memory_len=sentence_len)
...
#             tf.matmul(tf.reshape(concat_video_sentence_fts, [-1, 4 * Params.attn_size]), W_fuse) + b_fuse)
...
def Caption_Generator(img, caption_placeholder, mask, dim_in, dim_embed, dim_hidden, n_lstm_steps, n_words, init_b):
    # img = tf.reshape(tf.transpose(img, [0, 2, 1]), [-1, Params.max_video_len])
    img = tf.squeeze(tf.reduce_sum(img, 1))
...
def unsup_sequence_prototypes_net(video, video_len, sentence_state, cell_f, params, weights_mlp, bias_mlp,
                                  scope=None):
    weights_p = params
    ...
    p_logits_list = []
    p_logits_list_new = []
    for i in range(Params.max_choose_len):
        with tf.variable_scope('sequence_attention'):
            ...
            if i == 0:
                ...
                f_inputs = inputs
                f_direction = 'start'
                forward_position = np.zeros([Params.batch_size]) - 1
            else:
                f_direction = 'forward'
            ...
            with tf.variable_scope('forward'):
                ...
                forward_position = predicted_position
                p_logits_list_new.append(predicted_position)
                # f_inputs = [expand_video, f_state]
                p_logits_list.append(p_logits)
                current_position = tf.transpose(tf.stack([predicted_position]), [1, 0])
...
def unsup_inference_prototypes_pointer_net(i, video, video_len, forward_position, sentence_state, cell_f, inference_f_state,
    ...
    with tf.variable_scope('sequence_attention'):
        ...
        with tf.variable_scope('forward'):
            ...
            # new_expand_video = scores * expand_video
            # new_video_state = new_expand_video + expand_video
            # idx = tf.convert_to_tensor(np.arange(0, Params.batch_size), dtype=tf.int32)
            # _, f_o_state = cell_f(attention_pool, f_state)

    # return p_logits, f_o_state, new_video_state, tf.greater(i, tf.constant(0))
...
def new_sequence_pointer_net(video, video_len, forward_position_list, sentence_state, cell_f, params, weights_mlp,
                             bias_mlp,
                             scope=None):
    weights_p = params
    ...
    # zeros_video_padding = tf.zeros([video_shape[0], 1, video_shape[2]], tf.float32)
    expand_video = video
    new_expand_video = 0.0 * expand_video
    f_inputs = [expand_video, sentence_state]
    f_state = sentence_state
    p_logits_list = []
    p_logits_list_new = []
    p_logits = \
        new_mfb_attention_with_padding(f_inputs, Params.attn_size,
                                       weights_p, weights_mlp, bias_mlp,
                                       memory_len=video_len,
                                       scope="attention_f")
    #pdb.set_trace()
    one = tf.ones_like(p_logits)
    zero = tf.zeros_like(p_logits)
    new_p_logits = tf.where(p_logits < 0.5, x=zero, y=one)
    scores = tf.expand_dims(p_logits, -1)
    # pdb.set_trace()
    new_expand_video = new_expand_video + scores * expand_video
    # idx_list = tf.contrib.framework.argsort(p_logits_new, axis=1, direction='ASCENDING')
    ...
    # f_inputs = [new_video_state, f_state]
    # return tf.stack(p_logits_list)
    return p_logits, expand_video, new_expand_video, new_p_logits


def new_inference_sequence_pointer_net(i, video, video_len, forward_position, sentence_state, cell_f, inference_f_state,
                                       params, weights_mlp, bias_mlp, scope=None):
    weights_p = params
    ...
    # with tf.variable_scope('forward'):
        # pdb.set_trace()
    p_logits = new_mfb_attention_with_padding(f_inputs, Params.attn_size,
                                              weights_p, weights_mlp, bias_mlp,
                                              memory_len=video_len,
                                              current_position=forward_position,
                                              direction='forward',
    ...
    # one = tf.ones_like(p_logits)
    # zero = tf.zeros_like(p_logits)
    # p_logits = tf.where(p_logits < 0.5, x=zero, y=one)
    # scores = tf.expand_dims(p_logits, -1)
    # attention_pool = tf.reduce_sum(scores * expand_video, 1)
    # idx_list = tf.contrib.framework.argsort(p_logits, axis=1, direction='ASCENDING')
    # current_position = idx_list[:, -1]
...
def new_mfb_attention_with_padding(inputs, units, weights, weights_mlp, bias_mlp, scope, memory_len=None,
                                   current_position=None, direction=None,
                                   reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        # pdb.set_trace()
        ...
        scores = tf.reduce_sum(multilayer_perceptron(outputs, weights_mlp, bias_mlp), [-1])
        if memory_len is not None:
            scores = mask_video_attn_score(scores, memory_len)
        return tf.nn.softmax(scores)  # all attention output is softmaxed now


def mask_video_attn_score(seq_fts, memory_sequence_length):
    # pdb.set_trace()
    score_mask = tf.sequence_mask(memory_sequence_length, maxlen=Params.max_video_len)
    score_mask_zeros = tf.zeros([Params.batch_size, Params.max_video_len], tf.float32)
    score_mask_values = tf.ones_like(score_mask_zeros)
    seq_mask = tf.where(score_mask, score_mask_values, score_mask_zeros)
    # seq_mask = tf.expand_dims(seq_mask, -1)
    seq_fts_masked = seq_mask * seq_fts
...
def multilayer_perceptron(X, weights, bias, scope="MLP"):
    ...
    with tf.variable_scope(scope):
        ...
        layer1 = tf.nn.relu(tf.add(tf.matmul(X, weights[0]), bias[0]))  # Hidden layer with relu activation
        output = tf.nn.relu(tf.add(tf.matmul(layer1, weights[1]), bias[1]))  # Hidden layer with Relu activation
        # output = tf.matmul(layer1, weights[2]) + bias[2]
...
def mask_attn_score_with_padding_with_position(score, memory_sequence_length, current_sequence_position, direction,
    ...
    #fixed to 0/1
    # score_mask_padding = tf.ones([score.shape[0], 1], tf.int32)
    score_mask_padding = tf.zeros([score.shape[0], 1], tf.int32)
    score_mask_padding = tf.cast(score_mask_padding, tf.bool)
    score_mask = tf.concat([score_mask_pre, score_mask_padding], 1)
