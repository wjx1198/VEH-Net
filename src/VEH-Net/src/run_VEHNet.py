...
def train(sub_dir, logging, model_save_dir, result_save_dir):
    ...
    model = VEHNet(bias_init_vector, word_emb_init, is_training=True)
    ...
    with tf.device("/cpu:0"):
        if finetune:
            saver.restore(sess, '../model_newloss_finetune/GTP_softmaxscale150.0_dropout0.2_alphaChoose1.0_alphaRegular0.001_dimHidden256_batchsize20_numLayers3_numGCN2/model-6')
    ...
    for epoch in range(Params.n_epochs):
        ...
        for current_batch_file_idx in xrange(len(train_data)):
            ...
            #     })
            _, loss_val, choose_loss_f, regular_loss, cap_loss, probability_f, sentenceAtt, fbo, svit, fg = sess.run \
                    (
                    [train_op, model.loss, model.choose_loss_f, model.regular_loss, model.cap_loss,
                     model.probability_f,
                     model.sentence_attn, model.final_bidirectional_outputs, model.sentece_video_interaction_fts,
                     model.fts_gatt],
                    feed_dict={
                        model.video: current_video_feats,
                        model.sentence_index: current_caption_matrix,
            ...
            print(
                "loss = {:f} choose_loss_f = {:f} regular_loss = {:f} cap_loss = {:f}".format(loss_val, choose_loss_f,
                                                                                              regular_loss,
                                                                                              cap_loss))

        print("Epoch: {:d} done.".format(epoch))
        tStop_epoch = time.time()
        print('Current Epoch Mean loss forward: {:f}'.format(np.mean(loss_epoch_f)))
...
def test(model_save_dir, result_save_dir):
    ...
    model = VEHNet(bias_init_vector, word_emb_init, is_training=False)
    config = tf.ConfigProto(allow_soft_placement=True)
    config.gpu_options.per_process_gpu_memory_fraction = 0.2
    sess = tf.InteractiveSession(config=config)
    with tf.device("/cpu:0"):
    ...
    for epoch in range(7, 50):
        ...
        for current_batch_file_idx in xrange(len(val_data)):
            ...
            for jj in range(Params.max_choose_len + 1):
                ...
                #                    model.sentence_w_len: current_caption_length
                #                })
                batch_probability_f, batch_output_f_state, output_cond, fg, svif, fbo, vsgatt, statt \
                    = sess.run([model.probability_f_inference,
                                model.output_f_state,
                                model.output_cond,
                                model.fts_gatt,
                                model.sentece_video_interaction_fts,
