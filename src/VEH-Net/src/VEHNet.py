...
class VEHNet(object):
    ...
    def __init__(self, bias_init_vector, word_emb_init, is_training=True):
        ...
        self.indices = tf.placeholder(tf.float32, [Params.max_choose_len + 1, Params.batch_size,
                                                   Params.max_video_len + 1])  # (5+1, batch_size, timestep+1)
        # caption generation
        ...
        # self.mid_video_encoding = tf.layers.conv1d(self.video, 2*Params.attn_size, 6,  strides=6, padding='valid', name='convc3d')
    ...
    def outputs_train(self):
        ...
        # pdb.set_trace()
        self.probability_f, self.choose_index_list, self.expand_video = sequence_prototypes_net(
    ...
    def outputs_inference(self):
        ...
        self.probability_f_inference, self.output_f_state, self.output_cond = \
            inference_sequence_prototypes_net(self.inference_step, self.final_bidirectional_outputs, self.video_w_len,
    ...
    def caption_gene(self):
        self.cap_loss =Caption_Generator(self.choosed_vs_fts, self.sentence_matrix, self.sentence_mask, self.dim_in,
